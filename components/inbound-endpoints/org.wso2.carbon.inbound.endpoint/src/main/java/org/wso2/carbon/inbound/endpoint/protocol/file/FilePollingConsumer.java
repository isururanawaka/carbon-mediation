            ...
            try {
                ...
                if (retryCount >= maxRetryCount) {
                    ...
                    log.warn("Failed to resolve the file URI: " + VFSUtils.maskURLPassword(fileURI) + ", in attempt " +
                            retryCount + ", " + e.getMessage() + " Retrying in " + reconnectionTimeout + " milliseconds.");
                }
            }
            if (wasError) {
                try {
            ...
            if (fileObject.exists() && fileObject.isReadable()) {
                ...
                if (children == null || children.length == 0) {
                    ...
                    if (vfsProperties.getProperty(VFSConstants.TRANSPORT_FILE_INTERVAL) != null) {
                        try {
                            ...
                            log.warn("Invalid param value for transport.vfs.FileProcessInterval : " +
                                    vfsProperties.getProperty(VFSConstants.TRANSPORT_FILE_INTERVAL) + ". Expected numeric value.");
                        }
                    }
                    if (vfsProperties.getProperty(VFSConstants.TRANSPORT_FILE_COUNT) != null) {
                        try {
                            ...
                            log.warn("Invalid param value for transport.vfs.FileProcessCount : " +
                                    vfsProperties.getProperty(VFSConstants.TRANSPORT_FILE_COUNT) + ". Expected numeric value.");
                        }
                    }
                    ...
                    for (FileObject child : children) {
                        ...
                        } else if (log.isDebugEnabled() && strFilePattern != null && !child.getName().
                                getBaseName().matches(strFilePattern) && !isFailedRecord) {
                ...
                log.warn("Unable to access or read file or directory : " + VFSUtils.maskURLPassword(fileURI) + "." +
                        " Reason: " + (fileObject.exists() ? (fileObject.isReadable() ?
                        "Unknown reason" : "The file can not be read!") : "The file does not exists!"));
                return null;
            }
        } catch (FileSystemException e) {
